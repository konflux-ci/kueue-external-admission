...
// CheckConnection verifies that the connection to AlertManager is working properly.
// It uses the AlertManager API v2 status endpoint to check if the service is reachable and healthy.
//
// Parameters:
//   - ctx: Context for cancellation and timeout control
//
// Returns:
//   - error: Returns an error if the connection check fails, nil if successful
func (c *AlertManagerTestClient) CheckConnection(ctx context.Context) error {
	// Create parameters for the status check
	params := general.NewGetStatusParamsWithContext(ctx)

	// Call the AlertManager status endpoint
	resp, err := c.client.General.GetStatus(params)
	if err != nil {
		return fmt.Errorf("failed to check AlertManager connection: %w", err)
	}

	// Check if we got a successful response
	if resp == nil {
		return fmt.Errorf("AlertManager returned empty status response")
	}

	c.logger.Info("Successfully verified AlertManager connection")
